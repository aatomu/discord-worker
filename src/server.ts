...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
      ...
      if (interaction.type === discord.InteractionType.ApplicationCommand) {
        ...
        if (interaction.data.type === discord.ApplicationCommandType.ChatInput) {
          switch (interaction.data.name.toLocaleLowerCase()) {
            ...
            // MARK: slash:vc_link
            case 'vc_link': {
              const message: discord.RESTPostAPIWebhookWithTokenJSONBody = {
                embeds: [
                  {
                    title: 'Your Speech link',
                    description: `**Don't share URL!**\nURL: https://discord-worker.aatomu.workers.dev/voice?id=${interaction.application_id}&token=${interaction.token}`,
                    color: embedSuccess,
                  },
                ],
              ...
              async function defer() {
                ...
                console.log(body)
                const res = await resourceRequest(env, 'POST', `/webhooks/${interaction.application_id}/${interaction.token}`, { 'Content-Type': 'application/json' }, body)
                console.log(res.statusText, await res.text())
              }
              ctx.waitUntil(defer())
    ...
    switch (request.method) {
      ...
      case 'POST': {
        switch (url.pathname) {
          // MARK: post:voice_save
          case '/voice_save': {
            const data: speechToTextData = await request.json()
            console.log('Request', data)
            const body: discord.RESTPostAPIWebhookWithTokenJSONBody = {
              content: data.text,
            }

            return fetch(`${entryPoint}/webhooks/${data.id}/${data.token}/messages/@original`, {
              method: 'PATCH',
              headers: {
